...
if __name__ == "__main__":
    ...
    sub_folders = [x[0].replace("{}\\".format(main_folder),"") for x in os.walk(main_folder)]
    sub_folders.remove(main_folder)
    sub_folder_items = [x[2] for x in os.walk(main_folder)]
    sub_folder_items.remove([])
    sub_folders_and_items = list(zip(sub_folders,sub_folder_items))
    print(sub_folders_and_items)
    #print(sub_folder_items)
    folders = sub_folders
    #print(folders)
    extensions = [# TIFFs
                  "tif", "tiff",
...
def preprocess(folders, extensions = None,
    ...
    # Generate paths list
    paths = [os.path.join(os.path.abspath(""), main_folder,folder) for folder in folders]
    file_list = []
...
preprocessor = preprocess(folders, extensions, randomise=False)

for i in range(len(sub_folders_and_items)):
    for j in range(len(sub_folders_and_items[i][1])):
        images = next(preprocessor)
        pad = images[0]
        crop = images[1]
        stretch = images[2]
        print("Still alive...",sub_folders_and_items[i][0],sub_folders_and_items[i][1][j])
        cv2.imwrite("{}\{}\pad_{}".format(main_folder,sub_folders_and_items[i][0],sub_folders_and_items[i][1][j]), pad) # create pad
        cv2.imwrite("{}\{}\crop_{}".format(main_folder,sub_folders_and_items[i][0],sub_folders_and_items[i][1][j]), crop) # create crop
        cv2.imwrite("{}\{}\stretch_{}".format(main_folder,sub_folders_and_items[i][0],sub_folders_and_items[i][1][j]), stretch) # create stretch
